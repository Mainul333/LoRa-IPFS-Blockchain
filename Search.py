...
def verify(pk, msg,sig):
	hash1=sha256();
	hash1.update(msg.encode('UTF-8'));
	z1=int(hash1.hexdigest(),16);
...
class Client(tk.Frame):
    ...
    def lastTx(self, transactions):
        self.chatBox.insert(tk.END,
		"\n\n************************************Transaction Details*********************************")
        self.chatBox.insert(tk.END,
             "\n Tx Index: "+str(transactions[0])+
    ...
    def createWidgets(self):
        ...
        self.sendButton = tk.Button(self, text="Search",bg='blue',fg='white')
        self.sendButton.myName = "Search"
    ...
    def send(self,event):
        ...
        if CID=="Provide the CID" or CID=="CID cannot be empty":
            ...
            if len(CID)==66:
                try:
                    receipt=web3.eth.getTransactionReceipt(CID)
                    logs = contract.events.newDataAdded().processReceipt(receipt)[0]['args']
                    data=[logs['_tx_index'],logs['_cid'],logs['_url'],logs['_timestamp'],logs['_signature'],logs['_public_key']]
                    if data[1]:
                        Tx=[data[0],data[1],data[2],data[3],data[4].hex(),data[5].hex()]
                        self.lastTx(Tx)
                    else:
                       self.chatBox.insert(tk.END,"\n No result found")
                except Exception:
                ...
                try:
                    data=contract.functions.getData(CID).call()
                    if data[1]:
                        Tx=[data[0],data[1],data[2],data[3],data[4].hex(),data[5].hex()]
                        self.lastTx(Tx)
                    else:
